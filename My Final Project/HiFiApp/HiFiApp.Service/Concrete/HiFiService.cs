...
namespace HiFiApp.Service.Concrete
{
    public class HiFiService : IHiFiService
    {
        ...
        public async Task<Response<List<HiFiDto>>> GetActiveHiFiAsync(bool isActive = true)
        {
            List<HiFi> hiFis = await _hiFiRepository.GetActiveHiFiAsync(isActive);
            if (hiFis.Count==0)
            {
        ...
        {
            var hiFis = await _hiFiRepository.GetHiFiWithCategoriesAsync();
            if (hiFis.Count==0)
            {
        ...
        public async Task<Response<List<HiFiDto>>> GetHiFiByCategoryIdAsync(int categoryId)
        {
            var hiFis = await _hiFiRepository.GetHiFiByCategoryIdAsync(categoryId);
            if(hiFis.Count==0)
            {
        ...
        public async Task<Response<List<HiFiDto>>> GetHiFiWithCategoriesAsync()
        {
            var hiFis = await _hiFiRepository.GetHiFiWithCategoriesAsync();
            if (hiFis.Count==0)
            {
        ...
        public async Task<Response<List<HiFiDto>>> GetHomeHiFiAsync()
        {
            List<HiFi> hiFis = await _hiFiRepository.GetHomeHiFiAsync();
            if (hiFis.Count==0)
            {
        ...
        public async Task<Response<HiFiDto>> UpdateAsync(EditHiFiDto editHiFiDto)
        {
            ...
            updatedHiFi.HiFiCategories=editHiFiDto
                .CategoryIds
                .Select(categoryId => new HiFiCategory
                {
                    HiFiId=updatedHiFi.Id,
